...
def sparkline(values: list[float], height: int = 42) -> go.Figure:
    ...
    fig = go.Figure(go.Scatter(
        ...
        ("rgba(18,183,106,.08)" if color == GREEN else "rgba(240,68,56,.08)"),
        hoverinfo="skip",
    ))
    fig.update_layout(
...
def line_chart(series: pd.Series, title: str = "", height: int = 300,
    ...
    fig.update_layout(
        title=dict(text=title, font=dict(size=13, color=TEXT, family="Inter")),
...
def multi_line(df: pd.DataFrame, title: str = "", height: int = 320) -> go.Figure:
    palette = [NAVY, BLUE, "#7C8AA5", "#B54708", "#027A48", "#B42318"]
    fig = go.Figure()
